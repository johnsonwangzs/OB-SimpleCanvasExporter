...
- `pnpm release` 通过 ESLint（零错误 / 警告）、21 项单元测试、TypeScript 正式构建及 1.0.0 元数据检查，生成安装文件。
- 新增 `src/metadata.ts` 的 5 项单元测试：默认标题与空值回退、单行文本、作者 / 时间独立选项、固定本地时间与 ISO 瞬间、对后续输入和时钟变化的隔离。
- 新增 `pnpm test:metadata`，用最小 Obsidian 控件 / 生命周期适配器调用真实 `ExportModal`、`snapshot`、`exportCanvas` 和 `saveHTML`，生成并重新打开实际组装的 HTML。验证页面标题、浏览器标签和画布可访问名称一致；修改标题不改变输出路径。
- 验证两个开关默认关闭、作者输入的启用状态、禁用作者不进入 HTML、空作者省略、导出期间字段锁定、无效路径、写入失败后的状态恢复与重试。控件适配器不是 Obsidian 原生界面，不能据此宣称现场 UI 已验收。
- 以 `2026-09-18T02:03:04Z` 在上海时区导出，模拟写入等待 10 分钟后，显示仍是开始时的 `2026-09-18 10:03`；详细提示含秒与 `UTC+08:00`。随后在纽约时区、模拟 2030 年重新打开 HTML，显示时间保持不变。
- 标题和作者包含 HTML 结束标签、脚本、图片事件属性、引号、中文和 `&` 时，均按文本显示，无额外元素、脚本执行或远程请求。
- 中英文、深浅色下验证 1360 / 900 / 650 / 320px：标题、作者、时间从左至右处于同一行，时间完整，长标题 / 作者缩略且保留完整 title；页面无水平溢出，画布位于实际工具栏下方。搜索、阅读面板、背景换色均可继续使用；禁用 JavaScript 仍显示静态元数据，打印沿用隐藏工具栏的行为。
- 搜索、阅读面板、Badge、背景四个既有浏览器套件通过，包括可用的默认主题 / Prism 冻结样本。新旧测试均无脚本异常或自动远程请求。
...
专项结果与截图保存在 `qa/metadata/`；`modal-adapter.png` 是测试适配器界面，不是 Obsidian 现场截图。安装并重载 1.0.0 后重新导出，即可使用新选项。
