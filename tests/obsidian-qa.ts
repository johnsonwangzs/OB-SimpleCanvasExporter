...
// Included only in a --qa build used locally. Release builds eliminate this module.
export async function runQA(app:App):Promise<void> {
  ...
  try {
    ...
    const nativeMeasurements=result.metrics.map(metric=>{
      ...
      if(Math.abs(scrollDifference)>2||Math.abs(clientDifference)>1)throw Error(`Card layout changed: ${metric.id} (scroll ${scrollDifference}, client ${clientDifference}).`);
    ...
    await app.vault.adapter.write(`${base}/qa-behavior.json`,JSON.stringify({noOverwrite:true,cancellationCleanup:true,unsavedSnapshot:true,first,second},null,2));
