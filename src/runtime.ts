...
export interface NativePath { path:string; ends:SVGElement|undefined; line:SVGElement; bounds:Bounds }
export function matchingPath(native:NativeCanvas|undefined,e:CanvasEdge,a:CanvasNode,b:CanvasNode): NativePath | undefined {
  try {
    ...
    const line=edge?.lineGroupEl?.querySelector<SVGElement>('.canvas-display-path');
    const path=line?.getAttribute('d');
    if (!line || !path || !/^[\s\d.,+eEMmLlCcZz-]+$/.test(path)) return;
    ...
    return {path,line,ends:edge?.lineEndGroupEl,bounds:{minX:Math.min(...xs)-16,minY:Math.min(...ys)-16,maxX:Math.max(...xs)+16,maxY:Math.max(...ys)+16}};
  } catch { return undefined; }
}
