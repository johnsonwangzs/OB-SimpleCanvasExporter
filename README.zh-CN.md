...
4. 确认 vault 内的输出路径，可修改页面标题、勾选显示作者或导出时间、启用背景水印，然后点击导出。默认保存到 Canvas 旁边；同名文件自动变为 `name (2).html`，不会覆盖。
...
导出时间记录开始导出时的本地时间，显示为 `YYYY-MM-DD HH:mm`；悬停可查看秒和 UTC 偏移。HTML 内保存固定时间，重新打开、分享或换到另一时区不会变成查看时的时间。这些选项用于本次导出；1.2.1 起可显式保存作者和时间显示偏好，下次打开自动填入。打印仍沿用已有规则隐藏工具栏。

## 添加背景水印

1.2.0 在导出窗口中提供 **背景水印**，默认关闭。开启后填写水印文字，调整 **不透明度**（4%–16%，默认 8%），并查看实时预览。文字最多 40 个 Unicode 字符，空白或超长输入会提示修改；换行与连续空白归一化为单个空格。

水印使用 24px 系统字体，向右上方倾斜 30°，交错平铺；长文字自动增大间距。图案固定在画布视口，画布平移和缩放不改变字号或间距；现有背景换色会自动切换深色 / 浅色水印。水印在分组、连线、卡片下面，不透明底色会遮住它，透明区域自然透出；不影响选字、滚动、搜索、Badge 和阅读面板。

文字与样式随 HTML 保存，离线打开或禁用 JavaScript 时仍能显示。打印保留水印并覆盖完整未缩放场景，分页沿用画布布局。导出页没有水印编辑按钮；重新打开导出窗口时使用已保存偏好，没有保存时默认关闭。旧 HTML 需要重新导出。水印用于署名或使用范围提示，不能防止接收者编辑 HTML 或截取未含水印的卡片。

## 记住导出偏好

1.2.1 在导出窗口中提供 **保存为默认** 和 **清除默认设置**。填写常用的作者、时间显示开关和水印设置后，点击保存；同一 vault 中再次打开任意 Canvas 的导出窗口会自动填入，重启 Obsidian 后也会保留。页面标题和输出路径始终根据当前 Canvas 生成，实际导出时间每次重新记录。

- 只有点击 **保存为默认** 才会更新偏好；临时修改后直接导出、导出失败或取消均不会覆盖默认值。保存后再取消，本次保存仍有效。
- **清除默认设置** 只影响下次打开，本次填写保持不变。未保存偏好时，作者、时间和水印继续默认关闭。
- 关闭作者或水印后保存，已填文字仍保留在偏好中，方便以后再次启用；关闭的字段不会写进 HTML。
- 读取或保存失败会明确提示，仍可按本次填写继续导出；开启的水印必须通过原有非空及长度校验。

偏好由 Obsidian 保存在此 vault 的插件 `data.json` 中，不同 vault 各自独立。已有的插件配置同步工具可能同步该文件；插件自身不新增网络访问。浏览器中导出页面的背景色记忆仍独立处理。

## 搜索导出的 HTML

0.2.0 新导出的文件在顶部提供搜索框。输入关键词，所有命中文本卡片同时描边，正文关键词高亮；默认淡化其他卡片及相关连线，可关闭“淡化其他卡片”。搜索覆盖完整正文，包括需要滚动才能看到的文字、代码、Badge 和双链显示文字。

搜索按字面短语匹配，英文不区分大小写，不解释正则表达式；文件、网页的占位内容、分组标签及隐藏链接目标不参与搜索。计数分别显示命中卡片数和关键词出现次数。
...
1.1.0 支持导出分组边框和组名，保留原始坐标、尺寸、边框颜色、粗细、圆角和静态底色。嵌套分组显示在卡片及连线下方；分组内部空白处仍可拖动画布。

分组仅作静态显示，不导出折叠 / 展开的 `+/-` 控件和分组背景图片。组名不参与文本卡片搜索、Badge 筛选或阅读面板；顶部单独统计分组数量。旧 HTML 需要重新导出。
...
当前版本面向桌面 Obsidian。文件卡片、网页卡片暂时保留位置并显示占位内容；嵌入笔记、PDF、远程图片等给出占位和导出说明。动态插件组件、复杂 SVG、依赖伪元素的主题装饰不保证完整还原，导出文件不运行笔记里的脚本或 Obsidian 插件。
...
`pnpm test:preferences` 验证插件持久化接口与真实窗口代码的保存、重开、清除、失败恢复、关闭期间写入、未启用文字省略及中英文窄屏布局。交互规则见[偏好设计](docs/EXPORT-PREFERENCES-DESIGN.zh-CN.md)。
